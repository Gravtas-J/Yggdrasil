...
for msg in st.session_state.messages:
    if msg["role"] == "assistant":
        ...
        with st.chat_message(msg["role"]):
            st.write(msg["content"])
if prompt:
    thinker = open_file(Thinker_loc)
    Decision = [{'role': 'system', 'content': thinker}, {'role': 'user', 'content': prompt}]
    response = openai.ChatCompletion.create(model="gpt-3.5-turbo-0125", messages=Decision, temperature=0, max_tokens=4000)
    text = response['choices'][0]['message']['content']                    
    # st.write(text)
